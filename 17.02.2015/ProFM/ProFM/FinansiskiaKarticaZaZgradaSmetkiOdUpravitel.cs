...
namespace ProFM
{
    public partial class FinansiskiaKarticaZaZgradaSmetkiOdUpravitel : Form
    {
        ...
        public FinansiskiaKarticaZaZgradaSmetkiOdUpravitel(Form1 parent)
        {
            InitializeComponent();
            MdiParent = parent;
        }

        private void KarticaZaZgradaSmetkiOdUpravitel_Load(object sender, EventArgs e)
        {
        ...
        private void btnPrebaraj_Click(object sender, EventArgs e)
        {
            ...
            var queryIzvodi = from izvod in context.tblIzvodis
                              where izvod.ID_zgrada == intIdZgrada & izvod.ID_dobavuvac == 1
                              select izvod;
            ...
            {
                Transakcija t = new Transakcija() { datum = "31.12.2013", transakcija = "претходна состојба ", dolzi = 0, pobaruva = 0, saldo = Convert.ToInt32(zaostanatDolg.vkupenZaostanatDolg2013) };
            ...
            foreach (tblIzdadeniFakturiZaCistenje izdfakturi in queryIzdadeniFakturiCistenje)
            {
                ...
                //ova ke bidi koga ke se prebaruva za naredni godini i ko ke se zema od prvata izdadena faktura vo godinata zaostanatiot dolg, kako prethodna sostojba
                ...
                if (int.Parse(datum_izdFaktura[2].ToString()) >= int.Parse(datumOdKojKeSePrebaruva.ToString()) && int.Parse(datum_izdFaktura[2].ToString()) <= int.Parse(datumDoKojKeSePrebaruva.ToString()))
                {
                    /*if (brojac == 0)
                    {
                        //t = new Transakcija() { datum = datumDolg, transakcija = "претходна состојба ", dolzi = 0, pobaruva = 0, saldo = float.Parse(izdfakturi.zaostanatDolg.ToString()) };
                        //listTransakcii.Add(t);

                        t = new Transakcija() { datum = izdfakturi.datum_izdavanje, transakcija = "фактура број " + izdfakturi.br_faktura, dolzi = float.Parse(izdfakturi.vkupno_iznos.ToString()), pobaruva = 0 };
                        listTransakcii.Add(t);
                    }
                    else
                    {*/
                        t = new Transakcija() { datum = izdfakturi.datum_izdavanje, transakcija = "фактура број " + izdfakturi.br_faktura, dolzi = Convert.ToInt32(izdfakturi.vkupno_iznos), pobaruva = 0 };
            ...
            foreach (tblIzvodi izvod in queryIzvodi)
            {
                ...
                {
                    transakcija = new Transakcija() { datum = izvod.datum, transakcija = "уплата по повикувачки број" + izvod.povikuvacki_broj, dolzi = 0, pobaruva = Convert.ToInt32(izvod.iznos) };
            ...
            float iznos = listTransakcii[0].saldo;

            for (int i = 1; i < listTransakcii.Count; i++)
            {
                listTransakcii[i].saldo = Convert.ToInt32(iznos + listTransakcii[i].dolzi - listTransakcii[i].pobaruva);
        ...
        private void btnPecatenje_Click(object sender, EventArgs e)
        {
            ...
            tblPecatenjeFinansiskaKarticaZgradaUpravitel pecatenFI = new tblPecatenjeFinansiskaKarticaZgradaUpravitel()
            {
                ID_Zgrada = idZgrada,
            ...
            context.tblPecatenjeFinansiskaKarticaZgradaUpravitels.InsertOnSubmit(pecatenFI);
